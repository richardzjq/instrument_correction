...
/**
 * 远程数据实时同步线程客户端,负责上传数据 作者:feiyangqingyun(QQ:517216493) 2017-1-15
 * 1:可设置远程mysql数据库信息
 * 2:自动重连数据库
 * 3:可设置检测连接间隔
 * 4:自动重置远程设备信息
...
private:
    ...
    QStringList initSql;            //初始化临时表sql语句队列
...
public slots:
    ...
    //设置初始化用的sql语句
    void setInitSql(const QStringList &initSql);
