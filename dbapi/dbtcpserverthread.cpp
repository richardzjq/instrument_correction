...
void DbTcpServerThread::run()
{
    while (!stopped) {
        ...
        if (dbOkNet) {
            ...
            msleep(3000);
            openDb();
            emit debug(QString("重连云端下载数据库%1").arg(dbOkNet ? "成功" : "失败"));
            msleep(3000);
        }
    }
...
bool DbTcpServerThread::checkDb()
{
    ...
    emit debug(QString("检查云端下载数据库连接(共 %1 条/用时 %2 秒)").arg(1).arg(QString::number(ms / 1000, 'f', 3)));
...
void DbTcpServerThread::setCheckConn(bool checkConn)
{
